...
class Metric(ClassProvider):
    ...
    def __init__(self, config, dataset=None, name=None, state=None, profiler=None, transform=None):
        self.config = config
        self.name = name
    ...
    def evaluate(self, annotations=None, predictions=None):
    ...
    def batch_update(self, annotations, predictions):
        if self.out_transform:
            predictions = self.out_transform(predictions)
        results = []
        for ann, pred in zip(annotations, predictions):
            self.submit(ann, pred)
            results.append(self.value)
        return results

    @property
    def value(self):
        return {self.name: self._current_value} 

    @property
    def avg_value(self):
        if self._final_value is None:
            self._final_value = self.evaluate()
        return {self.name: self._final_value} 

    def set_transform(self, out_transform):
