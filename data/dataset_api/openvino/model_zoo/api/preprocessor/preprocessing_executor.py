...
class PreprocessingExecutor:
    def __init__(
            self, processors=None, preprocessing_configuration=None, dataset_name='custom', dataset_meta=None,
            input_shapes=None,
        ...
        if enable_ie_preprocessing:
            if not ie_preprocess_available():
                ...
                self.ie_processor = IEPreprocessor(preprocessing_configuration)
                processors = self.ie_processor.keep_preprocessing_info

        if not processors and not preprocessing_configuration:
            return

        if processors:
            self.processors = processors
        if preprocessing_configuration:
            identifier = 'type'
            for processor in preprocessing_configuration:
    ...
    def __call__(self, batch_data, *args, **kwargs):
        return self.process(batch_data, *args, **kwargs)
